...
def set_x_axis_invlog(ax,ranks,xtickrotation=0):
...
def set_y_axis_invlog(ax,ranks):
...
def set_frame(ax,rankmin=0,rankmax=99.99,axisIL='x',xtickrotation=0):
    ...
    #- set axis
    if axisIL == 'x':
        set_x_axis_invlog(ax_frame,ranks_frame,xtickrotation=xtickrotation)
    elif axisIL == 'y':
        set_x_axis_invlog(ax_frame,ranks_frame)
...
def subplot_ranks_invlog(ax,ranks,y,sl=slice(None,None),rankmin=0,rankmax=99.999,set_frame=True,\
    ...
    ax_frame = None
    if set_frame:
        ax_frame = set_frame(ax,rankmin=rankmin,rankmax=rankmax,xtickrotation=xtickrotation)
    ...
    if isinstance(y,list):
        for i in range(len(y)):
            ...
            h = show_data(ax,ranks[i],y[i],axisIL='x',rankmin=rankmin,rankmax=rankmax,c=c,alpha=a,linestyle=lt,linewidth=lw,label=lab)
            h_all.append(h)
    else:
        h_all = show_data(ax,ranks,y,axisIL='x',rankmin=rankmin,rankmax=rankmax,c=col,alpha=alpha,linestyle=ltype,linewidth=linewidth,label=labels)

    return ax_frame, h_all

def add_x_hatch(ax,ranks,i_xlim,color='gray',hatch='//',
...
def add_y_hatch(ax,ranks,i_ylim,color='gray',hatch='//',
